...
;(function($) {
	...
	var elem, opts, preloader, orig_pos, final_pos, busy = false, nr, zindex = 120, titleh = 0, shadow = 20, margin = 20, fx = $.extend($('<div/>')[0], { prop: 0 });
	...
	function zoomIn() {
		...
		$('<div id="imgbox-wrap-' + nr + '" class="imgbox-wrap"></div>')
			...
			.append('<img class="imgbox-img" id="imgbox-img-' + nr + '" src="' + preloader.src + '" alt="' + '" />')
			.appendTo('body');
		...
		if (detail.length > 0) {
			$('<div id="imgbox-tmp" class="imgbox-title" />').html(detail).css('width', final_pos.width).appendTo('body');

			titleh = $('#imgbox-tmp').outerHeight() +10;

			final_pos.height	+= titleh;
			final_pos.top		-= titleh > margin + shadow ? margin : margin * 0.5;

			$('#imgbox-tmp').remove();
			$('#imgbox-wrap-' + nr).append('<div class="imgbox-title">' + detail + '</div>');
		}
	...
	function zoomOut() {
		...
		if (opts.speedOut > 0) {
			...
			setTimeout(function() {
				$('#imgbox-wrap-' + nr).css('z-index', 120);
				$('#imgbox-bg-' + nr).css('z-index', 120);
		    }, opts.speedOut * 0.5);
	...
	function _clean_up() {
        ...
        zindex = zindex > 120 ? zindex - 2 : 120;
